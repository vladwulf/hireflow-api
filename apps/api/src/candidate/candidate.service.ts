...
export class CandidateService {
  ...
  async getCandidatesByJob(jobUuid: string): Promise<GetCandidate[]> {
    ...
    return this.prisma.candidate.findMany({ where: { jobId: job.id }, include: { score: true } });
  }
  ...
  async getCandidate(uuid: string): Promise<GetCandidate> {
    const candidate = await this.prisma.candidate.findUnique({ where: { uuid }, include: { score: true } });
  ...
  async scoreCandidate(uuid: string): Promise<GetCandidate> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Failed to parse AI scoring response');
    }
